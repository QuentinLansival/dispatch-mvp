...
export default function CamionsPage() {
  ...
  function importData() {
    ...
    const mapped: Truck[] = rows
      .map(r => {
        ...
        const ie = (r["Interne/Externe"]?.trim().toUpperCase() === "EXT") ? "EXT" : "INT";
  ...
  function edit(i: number, key: keyof Truck, value: string) {
    ...
    else if (key === "hasCrane") t.hasCrane = value.toLowerCase() === "oui" || value.toLowerCase() === "true";
    else if (key === "internalExternal") t.internalExternal = (value.toUpperCase() === "EXT" ? "EXT" : "INT");
    else t[key] = value as any;
  ...
  return (
    <div>
      <h1>Camions</h1>
      <p>Colle ton tableau Excel (copier/coller) ou un CSV. Colonnes attendues : <b>ID, Interne/Externe, Label, Grue, Ml max</b></p>
      ...
      <table border={1} cellPadding={6} style={{ borderCollapse: "collapse", width: "100%" }}>
        ...
        <tbody>
          {trucks.map((t, i) => (
            <tr key={t.id}>
              <td><input value={t.id} onChange={e => edit(i, "id", e.target.value)} /></td>
              <td><input value={t.internalExternal} onChange={e => edit(i, "internalExternal", e.target.value)} /></td>
              <td><input value={t.label} onChange={e => edit(i, "label", e.target.value)} /></td>
              <td><input value={String(t.hasCrane)} onChange={e => edit(i, "hasCrane", e.target.value)} /></td>
              <td><input value={String(t.mlMax)} onChange={e => edit(i, "mlMax", e.target.value)} /></td>
            </tr>
          ))}
          {!trucks.length && (
            <tr><td colSpan={5}>Aucun camion importé.</td></tr>
          )}
        </tbody>
      </table>
    </div>
